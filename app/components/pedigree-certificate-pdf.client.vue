<template>
  <div>
    ...
    <div ref="certificateRef" class="certificate-container">
      <div class="certificate-wrapper">
        ...
        <div class="certificate-content">
          <!-- Title -->
          <h1 class="certificate-title">شهادة النسب</h1>
    ...
    >
      <span>{{ isGenerating ? 'جاري التحضير...' : 'طباعة شهادة النسب PDF' }}</span>
    </button>
  </div>
</template>
...
async function generatePDF() {
  ...
  try {
    ...
    // Name (bold)
    textCtx.font = 'bold 48px "Mohammad Bold Art", sans-serif'
    textCtx.fillText(props.name || ' ', CERTIFICATE_WIDTH / 2, 320)
    ...
    const drawJustifiedTextRTL = (text: string, y: number) => {
      ...
      if (lastWordWidth === undefined || lastPos === undefined) return
      ...
      if (Math.abs(actualDiscrepancy) > 0.5) {
        ...
        if (finalLastPos !== undefined) {
          const finalLastWordLeftEdge = finalLastPos - lastWordWidth
      ...
      for (let i = 0; i < words.length; i++) {
        ...
        if (!word || pos === undefined) continue
    ...
    textCtx.fillText('وقد طبعت هذه الشهادة استناداََ الي الوارد في كتاب القول الحاسم في نسب القواسم', CERTIFICATE_WIDTH - RIGHT_MARGIN, 650)
    const dateText = `طُبعت بتاريخ: ${currentDate.value} م`
    textCtx.fillText(dateText, CERTIFICATE_WIDTH - RIGHT_MARGIN, 680)
